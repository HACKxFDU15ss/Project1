...
int SSD::detect(const cv::Mat& img)
{
	__TIC__();
	preprocess(img);
...
#if CPU_ONLY
	getResultsCPU(output);
#else
	getResultsGPU(output);
#endif
	...
	/*
	 * mjt added
	 * aim: get the main picture tag which is detected
	*/

//---------------------

	caffe::Blob<float>* detections = output[0];
	const int num_det = detections->height();


	const float* detections_data = detections->cpu_data();
	...
	for (int i = 0; i < num_det; ++i) {
		...
		printf("img_idx:%d\tlabel:%d\tscore:%f\tlabel_name%s\n", img_idx, label, score, label_to_display_name_[label]);
	}
	return ret;
...
void SSD::getResultsCPU(const std::vector<caffe::Blob<float>*>& top)
{
	assert(top.size() == 1);
	...
	const float* conf_data = conf_blob->cpu_data();
	...
	const float* prior_data = prior_blob->cpu_data();
	...
	int num_loc_classes = config_.share_location ? 1 : config_.num_classes;
	std::vector<caffe::LabelBBox> all_loc_preds;
	caffe::GetLocPredictions(loc_data, 
		num, 
		num_priors, 
		num_loc_classes, 
	...
	std::vector<caffe::NormalizedBBox> prior_bboxes;
	std::vector<std::vector<float> > prior_variances;
	caffe::GetPriorBBoxes(prior_data, 
		num_priors, 
		&prior_bboxes, 
		&prior_variances);
	...
	caffe::DecodeBBoxesAll(all_loc_preds, 
		prior_bboxes, 
		prior_variances, 
		num, 
		config_.share_location, 
	...
	for (int i = 0; i < num; ++i)
	{
		...
		if (config_.keep_top_k > -1 && num_det > config_.keep_top_k)
		{
			...
			std::sort(score_index_pairs.begin(), 
				...
				caffe::SortScorePairDescend<std::pair<int, int> >);
...
// top blob shape: [n, c, h, w] = [1, 1, N, 7];
// which N is the number of objects.
void SSD::getResultsGPU(const std::vector<caffe::Blob<float>*>& top)
{
	assert(top.size() == 1);
	boost::shared_ptr<caffe::Blob<float> >  loc_blob = net_->blobs()[loc_blob_idx_];
